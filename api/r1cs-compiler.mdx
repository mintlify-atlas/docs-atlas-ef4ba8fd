...
### NoirProofSchemeBuilder

Trait for constructing proof schemes from Noir programs.
...
<ParamField path="from_file" type="fn(path) -> Result<Self>">
  Loads a Noir program from a compiled JSON artifact file and builds a complete proof scheme
</ParamField>

<ParamField path="from_program" type="fn(program) -> Result<Self>">
  Constructs a proof scheme from a pre-loaded `ProgramArtifact`
</ParamField>
...
// Load from compiled Noir program
let scheme = NoirProofScheme::from_file("./target/basic.json")?;

// This compiles the ACIR to R1CS and sets up witness generators
println!("Constraints: {}", scheme.r1cs.num_constraints());
```

### WhirR1CSSchemeBuilder

Trait for constructing WHIR proof schemes optimized for R1CS instances.
...
<ParamField path="new_for_r1cs" type="fn(r1cs, w1_size, num_challenges, has_public_inputs) -> Self">
  Creates a WHIR scheme configured for a specific R1CS instance with security parameters
</ParamField>

<ParamField path="new_whir_zk_config_for_size" type="fn(num_variables, num_polynomials) -> WhirZkConfig">
  Generates optimal zero-knowledge configuration based on instance size
</ParamField>

**Parameters:**

- `w1_size` - Size of first folding step (affects proof size and proving time)
- `num_challenges` - Number of sumcheck challenges (affects soundness error)
- `has_public_inputs` - Whether the R1CS has public inputs requiring special handling
...
// Configure WHIR for R1CS
let whir_scheme = WhirR1CSScheme::new_for_r1cs(
    &r1cs,
    20,    // w1_size: first fold to 2^20
    30,    // num_challenges: 30 sumcheck rounds
    true,  // has_public_inputs
);

// Get zero-knowledge configuration
let zk_config = WhirR1CSScheme::new_whir_zk_config_for_size(
    r1cs.num_witnesses(),
...
- Builders: `provekit/r1cs-compiler/src/noir_proof_scheme.rs:95-100`, `provekit/r1cs-compiler/src/whir_r1cs.rs:37-47`
